...
function site_settings_row(): array
{
    ...
    return $row ?: [
        ...
        'shared_agnes_api_name' => DEFAULT_AGNES_API_NAME,
        'shared_agnes_api_base_url' => DEFAULT_AGNES_API_BASE_URL,
        'shared_agnes_model' => DEFAULT_AGNES_IMAGE_MODEL,
        'shared_agnes_request_timeout' => DEFAULT_REQUEST_TIMEOUT,
        'shared_agnes_api_key_hint' => '',
    ];
}
...
function shared_agnes_api_enabled(): bool
{
    try {
        $row = site_settings_row();
        return !empty($row['shared_api_enabled']) && !empty($row['shared_agnes_api_enabled']);
    } catch (Throwable $error) {
        return false;
    }
}
...
function public_site_flags(): array
{
    ...
    return [
        ...
        'sharedAgnesApiEnabled' => !empty($row['shared_api_enabled']) && !empty($row['shared_agnes_api_enabled']),
...
function shared_api_category_client(array $row, string $category): array
{
    if ($category === 'agnes') {
        return [
            'apiName' => trim((string) ($row['shared_agnes_api_name'] ?? '')) ?: DEFAULT_AGNES_API_NAME,
            'apiBaseUrl' => trim((string) ($row['shared_agnes_api_base_url'] ?? '')) ?: DEFAULT_AGNES_API_BASE_URL,
            'model' => trim((string) ($row['shared_agnes_model'] ?? '')) ?: DEFAULT_AGNES_IMAGE_MODEL,
            'requestTimeout' => normalize_request_timeout($row['shared_agnes_request_timeout'] ?? DEFAULT_REQUEST_TIMEOUT),
            'hasApiKey' => shared_agnes_api_enabled() && !empty($row['shared_agnes_api_key_ciphertext']),
            'apiKeyHint' => shared_agnes_api_enabled() ? (string) ($row['shared_agnes_api_key_hint'] ?? '') : '',
        ];
    }
...
function shared_api_config_client(?array $row = null): array
{
    ...
    $agnesApi = shared_api_category_client($row, 'agnes');
    ...
    return [
        ...
        'promptApi' => $promptApi,
        'agnesApi' => $agnesApi,
        'hasAnyApiKey' => $imageApi['hasApiKey'] || $promptApi['hasApiKey'] || $agnesApi['hasApiKey'],
        'sortOrder' => -1,
        'isShared' => true,
    ];
}
...
{
    $row = $row ?? site_settings_row();
    if ($category === 'agnes') $prefix = 'shared_agnes_';
    elseif ($category === 'prompt') $prefix = 'shared_prompt_';
    else $prefix = 'shared_';
    return [
...
function admin_site_settings_view(): array
{
    ...
    return [
        ...
        'registrationEnabled' => !empty($row['registration_enabled']),
        'sharedApiEnabled' => !empty($row['shared_api_enabled']),
        'sharedAgnesApiEnabled' => !empty($row['shared_agnes_api_enabled']),
        'promptToolsEnabled' => !empty($row['prompt_tools_enabled']),
...
function save_site_settings(array $body): array
{
    ...
    $sharedAgnesApiEnabled = !empty($body['sharedAgnesApiEnabled']) ? 1 : 0;
    ...
    $agnesInput = shared_api_category_input($shared, 'agnes', $current['agnesApi']);
    ...
    if (!valid_api_base_url($agnesInput['apiBaseUrl'])) json_response(['error' => 'Agnes API 地址必须是 http 或 https 地址'], 400);
    ...
    $agnesApiFields = shared_api_key_storage_fields($agnesInput, $row, 'agnes');

    $stmt = $db->prepare('UPDATE site_settings SET wall_require_login = ?, registration_enabled = ?, shared_api_enabled = ?, shared_agnes_api_enabled = ?, prompt_tools_enabled = ?, shared_api_name = ?, shared_api_base_url = ?, shared_model = ?, shared_request_timeout = ?, shared_api_key_ciphertext = ?, shared_api_key_iv = ?, shared_api_key_tag = ?, shared_api_key_hint = ?, shared_prompt_api_name = ?, shared_prompt_api_base_url = ?, shared_prompt_model = ?, shared_prompt_request_timeout = ?, shared_prompt_api_key_ciphertext = ?, shared_prompt_api_key_iv = ?, shared_prompt_api_key_tag = ?, shared_prompt_api_key_hint = ?, shared_agnes_api_name = ?, shared_agnes_api_base_url = ?, shared_agnes_model = ?, shared_agnes_request_timeout = ?, shared_agnes_api_key_ciphertext = ?, shared_agnes_api_key_iv = ?, shared_agnes_api_key_tag = ?, shared_agnes_api_key_hint = ? WHERE id = 1');
    $stmt->execute([
        $wallRequireLogin,
        $registrationEnabled,
        $sharedApiEnabled,
        $sharedAgnesApiEnabled,
        $promptToolsEnabled,
        $imageInput['apiName'],
